	...
	constructor(place, roomcode, onstart) {
		...
		if (this.online) {
			try {
				...
				this.playersindex = {};
				// Open websocket
				ws = new WebSocket(`${document.location.protocol === "http:" ? "ws" : "wss"}://${document.location.host}${document.location.pathname.slice(0, document.location.pathname.lastIndexOf("/"))}/server.js?${encodeURIComponent(Settings.NAME)}&${Settings.COLOR}` + (roomcode ? `&${roomcode}` : ""));
				ws.onopen = () => {
